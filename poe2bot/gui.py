...
class App(tk.Tk):
    ...
    def _build_widgets(self):
        ...
        self.tree = ttk.Treeview(
            right, columns=("key", "delay", "jitter", "hold"), show="headings", height=10)
        for col, label, width in (
            ("key", "Key", 80), ("delay", "Delay (ms)", 90),
        ...
        for label, var, width in (
            ("Key", self.step_key_var, 8), ("Delay", self.step_delay_var, 6),
        ...
        self.step_ready_timeout_var = tk.StringVar(value="5000")
        ...
        for text, cmd in (
            ("Add Step", self._add_step), ("Update Selected", self._update_selected_step),
    ...
    def _reset_ready_form(self):
        ...
        self.step_ready_timeout_var.set("5000")
    ...
    def _refresh_steps_tree(self):
        ...
        for i, step in enumerate(self.editing_steps):
            self.tree.insert("", tk.END, iid=str(i),
                              values=(step.key, step.delay_ms, step.jitter_ms, step.hold_ms))
    ...
    def _read_step_form(self):
        try:
            ...
            messagebox.showerror(
                "Invalid step",
                "Delay/jitter/hold/timeout must be whole numbers, and confidence must "
                "be a decimal (e.g. 0.9).")
            return None
        return Step(
